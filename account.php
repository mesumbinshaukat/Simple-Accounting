...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $transferTo = isset($_POST['transfer_to']) ? (int)$_POST['transfer_to'] : 0;
  if ($amount <= 0) {
  ...
  if ($action === 'credit' || $action === 'debit') {
    ...
    if ($stmt->execute()) {
      ...
      // Optional deposit to another account for DEBIT action
      // When we take (debit) from current account and give to someone, credit the target account
      if ($action === 'debit' && $transferTo > 0 && $transferTo !== $id) {
        $target = find_account($transferTo);
        if ($target) {
          $note2 = $note !== '' ? $note : ('Transfer from ' . $account['name']);
          ...
          $stmt2->execute();
          add_flash('Credited to account: ' . e($target['name']), 'success');
        }
      }
    } else {
  ...
  <div class="grid grid-2">
    <div class="card">
      ...
      <form method="post">
        ...
        <button class="btn" type="submit" style="margin-top:10px;">Meine Diya (Add Credit)</button>
      </form>
    </div>
    <div class="card">
      ...
      <form method="post">
        ...
        <div style="margin-top:10px;">
          <label style="display:flex;align-items:center;gap:8px;">
            <input type="checkbox" id="depositToggle"> Credit another account
          </label>
          <div id="depositWrap" style="display:none;margin-top:8px;">
            <input type="hidden" name="transfer_to" id="transferTo">
            <input class="input" type="text" id="accountSearch" placeholder="Search account to credit..." data-role="account-search" data-exclude-id="<?= (int)$id; ?>">
            <div class="dropdown-list" id="accountSearchList" style="display:none;"></div>
          </div>
        </div>
        <button class="btn secondary" type="submit" style="margin-top:10px;">Meine Liya (Add Debit)</button>
      </form>
    </div>
  </div>
